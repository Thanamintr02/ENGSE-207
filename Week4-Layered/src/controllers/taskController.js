...
class TaskController {
    ...
    // ===============================================
    // ** ฟังก์ชันที่ถูกเพิ่มเข้ามาเพื่อแก้ไขปัญหา **
    // ===============================================

    /**
     * PATCH /api/tasks/:id/status
     * อัพเดทสถานะ task โดยตรงตามที่ Frontend เรียกใช้
     */
    async updateTaskStatus(req, res, next) {
        try {
            const id = parseInt(req.params.id);
            const { status } = req.body; // ดึงสถานะใหม่จาก body
            
            if (isNaN(id) || !status) {
                return res.status(400).json({
                    success: false,
                    error: 'ID หรือสถานะใหม่ไม่ถูกต้อง'
                });
            }

            // ใช้ Service updateTask เดิม เพื่ออัปเดตเฉพาะสถานะ
            const updates = { status: status.toUpperCase() }; // แปลงเป็นตัวพิมพ์ใหญ่เพื่อความชัวร์ (ถ้าจำเป็น)

            // ใช้ service ตัวเดิมที่ถูกออกแบบมาสำหรับการอัปเดต
            const task = await taskService.updateTask(id, updates); 
            
            // ส่ง Response 200 OK กลับไป
            res.status(200).json({
                success: true,
                data: task,
            ...
            // Log Error เพื่อหาปัญหา 500
            console.error('❌ TaskController Error (updateTaskStatus):', error); 
            next(error);
        }
    }
